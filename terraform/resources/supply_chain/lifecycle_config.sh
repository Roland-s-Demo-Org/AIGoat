 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "    features[img_key] = extract_features(img_data).tolist()\\n",
    "\\n",
    "# Save features to a file\\n",
    "with open('image_features.json', 'w') as f:\\n",
    "    json.dump(features, f)\\n",
    "\\n",
    "s3.upload_file('image_features.json', bucket_name, 'image_features.json')\\n",
    "\\n",
    "# Load precomputed features\\n",
    "with open('image_features.json', 'r') as f:\\n",
    "    features_data = json.load(f)\\n",
    "    features = {k: np.array(v) for k, v in features_data.items()}\\n",
    "\\n",
