...
resource "aws_instance" "backend" {
  depends_on = [aws_db_instance.rds, aws_security_group.rds_sg, aws_security_group.ec2-sg, aws_secretsmanager_secret_version.db_credentials]
  ...
  write_files:
    - path: /home/ec2-user/setup.sh
      ...
      content: |
        ...
        SECRET_JSON=$(aws secretsmanager get-secret-value --secret-id ${aws_secretsmanager_secret.db_credentials.id} --region ${data.aws_region.current.name} --query SecretString --output text)
        ...
        sudo nohup python3 app.py --db_user=$DB_USER --db_password=$DB_PASSWORD --db_host=$DB_HOST --db_name=$DB_NAME --comments_api_gateway=${var.output_integrity_api_endpoint} --similar_images_api_gateway=${var.supply_chain_api_endpoint} --similar_images_bucket=${var.supply_chain_bucket_name} --get_recs_api_gateway=${var.data_poisoning_api_endpoint} --data_poisoning_bucket=${var.data_poisoning_bucket_name} &
  runcmd:
