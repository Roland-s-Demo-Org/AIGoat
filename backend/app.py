...
def product_lookup(current_user):
    ...
    if request.method == 'POST':
        ...
        with open(photo_path, 'rb') as img_file:
            ...
            # Extract metadata safely - process_image now returns structured metadata
            # instead of executing commands
            image_metadata = process_image(image_data)

        # Log metadata for debugging purposes only - never execute or return to client
        if image_metadata:
            app.logger.info(f"Extracted image metadata: {image_metadata}")
            # Metadata is logged but not returned to prevent information disclosure
